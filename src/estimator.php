...
//    $data = json_encode($datas, JSON_FORCE_OBJECT);
    ...
    if($data["periodType"] =="days") {
        $IdollarsInFlight = $impactinfectionsByRequestedTime * 0.65 * 0.15 * $data["timeToElapse"];
        $SdollarsInFlight = $severeimpactinfectionsByRequestedTime * 0.65 * 0.15 * $data["timeToElapse"];
    }
    elseif ($data["periodType"] =="weeks"){
        $IdollarsInFlight = $impactinfectionsByRequestedTime * 0.65 * 0.15 * $data["timeToElapse"] * 7;
        $SdollarsInFlight = $severeimpactinfectionsByRequestedTime * 0.65 * 0.15 * $data["timeToElapse"] *7;
    }
    elseif ($data["periodType"] =="months"){
        $IdollarsInFlight = $impactinfectionsByRequestedTime * 0.65 * 0.15 * $data["timeToElapse"] * 30;
        $SdollarsInFlight = $severeimpactinfectionsByRequestedTime * 0.65 * 0.15 * $data["timeToElapse"] * 30;
    }
    else{
        $IdollarsInFlight = 0;
        $SdollarsInFlight = 0;
